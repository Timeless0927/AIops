...
async def test_publish_approval_execution_result_replies_to_incident_thread(
    ...
    result = await module.publish_approval_execution_result(
        ...
        },
        {
            ...
            "resource_name": "api",
            "audit_id": 3,
            "health_result": {"summary": "deployment rollout and replicas healthy"},
        },
        {"platforms": {"feishu": {"main_chat_id": "oc_ops"}}},
    )
    ...
    assert "审批执行结果: 成功" in calls[0][1]["content"]
    assert "deployment rollout and replicas healthy" in calls[0][1]["content"]
    assert result == {

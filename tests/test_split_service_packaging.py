...
def test_gateway_image_defaults_include_alertmanager_handoff_env() -> None:
    ...
    assert "AIOPS_DIAGNOSIS_URL=http://diagnosis:8082" in dockerfile
    assert "AIOPS_DIAGNOSIS_PATH=/diagnosis/sessions" in dockerfile
...
def test_dockerfile_does_not_copy_entire_repository_into_service_images() -> None:
    ...
    service_copy_boundaries = {
        ...
        "diagnosis": (
            ...
            "COPY toolsets/__init__.py toolsets/incident_diagnosis.py toolsets/k8s_redact.py /app/toolsets/",
...
def test_compose_smoke_wires_gateway_diagnosis_and_connectors() -> None:
    ...
    assert services["diagnosis"]["build"]["target"] == "diagnosis-smoke"
    assert services["smoke"]["build"]["target"] == "diagnosis-smoke"
...
def test_split_service_entrypoints_forward_explicit_commands() -> None:
    for script in (
        ...
        "deploy/entrypoint-diagnosis.sh",
...
def test_gateway_entrypoint_sets_alertmanager_handoff_defaults(tmp_path: Path) -> None:
    ...
    script.write_text(
        "#!/usr/bin/env bash\n"
        f"printf '%s\\n' \"$AIOPS_CONNECTOR_URL\" \"$AIOPS_DIAGNOSIS_URL\" \"$AIOPS_DIAGNOSIS_PATH\" > {log_path}\n",
        encoding="utf-8",
    )
    script.chmod(0o755)

    env = os.environ.copy()
    for name in ("AIOPS_CONNECTOR_URL", "AIOPS_DIAGNOSIS_URL", "AIOPS_DIAGNOSIS_PATH"):
        env.pop(name, None)
...
def test_diagnosis_smoke_connectivity_to_gateway() -> None:
    ...
    try:
        ...
        for process in (diagnosis, gateway):
            process.terminate()
            try:
                process.wait(timeout=5)
            except subprocess.TimeoutExpired:

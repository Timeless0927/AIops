...
diagnosis_service/     # diagnosis service: service_main.py + diagnosis orchestration
hermes/                # one-window compatibility shim only
...
- **One process per `apps/<service>/`**, each with its own `main()` calling
  ...
  `diagnosis_service/service_main.py`.
...
- **No service imports another service's internals.** Gateway talks to Connector
  ...
  env vars (`AIOPS_PROMETHEUS_MCP_URL`, etc., in `diagnosis_service/service_main.py`).
...
- **`toolsets/` holds durable stores + legacy diagnosis-tool registry.** New Gateway
  stores (e.g. approval) live as a service module (`apps/aiops_k8s_gateway/approval_service.py`),
  not in `toolsets/`, unless they are shared by diagnosis tools too.

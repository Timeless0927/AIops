...
## Deploy Overlay → Runtime Env → Evidence Collection Boundary

Switching `dev-external` is a cross-layer handoff: overlay ConfigMap → pod env
(read once at process start) → MCP HTTP → Hermes `_collect_evidence` →
`incident_evidence` row. Each arrow can silently fail.

### Checklist: After Switching An Observability Overlay

- [ ] Verify the in-pod env, not just `kubectl kustomize` — base Deployments
      have no ConfigMap checksum annotation, so `apply` does not restart pods;
      `rollout restart` and re-`exec echo $PROMETHEUS_URL` before trusting
      evidence (see `deploy/dev-external-observability-contract.md` rollout note)
- [ ] Verify backend reachable *and* carries the scope namespace's data
      (Prometheus `up{namespace=…}`, Loki `label/namespace/values`), not just
      `status:success`
- [ ] For a `dev-external` evidence smoke, assert non-empty payload +
      non-`failed` session + `incident_analysis` written — a single empty
      `logs` row with `line_count:0` is the silent-failure signature, not a
      pass
- [ ] If a channel is empty, check the prerequisite owned by another layer
      (app stdout for Loki, ServiceMonitor for Prometheus), not the overlay

**Real-world example**: `dev-external` pointed at a real `loki`-ns backend,
env rollout verified, backends reachable — yet the ADR-0005 Issue A smoke
persisted only one `logs` evidence row with `line_count:0` and a `failed`
session. Root cause was three independent gaps in *other* layers: AIOps pods
emitted no stdout (Loki had nothing to scrape), no ServiceMonitor covered
`aiops-dev` (Prometheus `up` had no aiops series), and `run_diagnosis_session`
marked the session `failed` on the empty backend. The overlay was correct;
the smoke still failed — evidence of correctness must be read at the
terminus (`incident_evidence` + `investigate_end`), not at the overlay.

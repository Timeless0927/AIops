...
- base 放 ServiceMonitor 会影响所有 overlay 含 dev-bundled —— dev-bundled 的 bundled
  Prometheus 是 inline 没 `serviceMonitorSelector`,SM 不会被它 pick,无损(但 apply 需 CRD 在,
  见 Open items 4);dev-disabled 更无害。离线 `kubectl kustomize` dev-bundled 渲染 exit 0。

## Open items(集群侧关闭,代码侧已了)

1. 集群实测 Prometheus `serviceMonitorSelector` —— **HARD GATE**:必须在集群确认后改
   `deploy/k8s/base/servicemonitor.yaml` 的 `metadata.labels.release` 值与其它 selector label,
   否则 SM 不被 Prometheus pick。`servicemonitor.yaml` 顶部已注释 GATE 标志 + 一条 kubectl
   探测命令。本会话从代码工作站无法跑集群命令,该值是 `prometheus-stack` 占位,集群侧必须改。
2. `serviceMonitorNamespaceSelector`:Prometheus 在 `loki` ns、SM 在 `aiops-dev` ns —— 跨 ns
   discovery 需 Prometheus 允许;集群侧确认,否则 SM 要搬到 `loki` ns。
3. hermes `APP_NAME`:确认 hermes `service_main.py` 全文硬编码 `"hermes"`(无 `APP_NAME` 常量),
   `write_metrics("hermes")` 与既有 `{"service": "hermes"}` 模式一致。**已关闭**。
4. **CRD 前置**:`ServiceMonitor` 是 `monitoring.coreos.com/v1` CRD,集群若无 Prometheus Operator
   则 `kubectl apply` 会被拒绝(dev-external 目标集群有 kube-prometheus-stack 故有 CRD;但
   dev-bundled 若 applied 到一个没装 Operator 的集群也会带这个 SM 且可能报错 —— 离线渲染仍
   `kubectl kustomize` exit 0,但 apply 需 CRD 在)。集群侧重 apply 前确认。

## Rollout / Rollback shape

- 代码:加 helper + 各 do_GET 一行回退(删即回)。
- 部署:`apply -k dev-external` 后 ServiceMonitor 落地;回滚删 SM 即可(Prometheus 自动摘
  target,`up` 回空)。
- 无 schema/durable 变更,无数据迁移。

## Implementation status(代码侧完成,集群侧待验)

- ✅ Step 1 (`apps/service_http.py` `metrics_body` + `JsonHandler.is_metrics_request`/`write_metrics`)。
- ✅ Step 2 (`gateway`/`hermes`/`connector`/`observability_http` do_GET 顶 `is_metrics_request` 早返回;
  `observability_http` 一处覆盖 mcp-prometheus/loki/topology 三个服务)。
- ✅ Step 3 (Service/Deployment 不增端口,SM 指现有 `http` 命名端口)。
- ✅ Step 4 (`deploy/k8s/base/servicemonitor.yaml` + `base/kustomization.yaml` `resources:` 追加;
  `kubectl kustomize deploy/k8s/overlays/dev-external` 及 `dev-bundled` 渲染均 exit 0、SM 正确出)。
- ✅ 自检 `python3 apps/service_http.py` → `ok: access+metrics; metrics: True`;import observability
  handler + `metrics_body` 字节正确;`test_observability_mcp_runtime.py`+gateway+hermes+connector 31 passed。
- ✅ Step 5/6 (集群):deploy apply + rollout restart 6 服务、`up{namespace="aiops-dev"}` 6 个 target 全 up。ServiceMonitor 被 Prometheus 接受,metrics 路非空。
...
from .evidence_service import redact
...
class AgentRunDB:
        ...
        def _write(conn: sqlite3.Connection) -> None:
            ...
            conn.execute(
                ...
                INSERT INTO agent_runs (
                    run_id, conversation_id, status, scope_json, incident_id, created_by, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (run_id, conversation_id, "running", _json_dumps(scope), incident_id, actor_id, now, now),
            )
            _insert_event(conn, run_id, "run_created", "mainline", "Run created", {"scope": scope, "tags": tags}, now, actor_id, None)
            _insert_event(conn, run_id, "run_started", "mainline", str(payload.get("message") or "Investigation started"), {"incident_id": incident_id}, now, actor_id, None)
    ...
    async def list_runs(self) -> list[JSON]:
        def _read() -> list[JSON]:
            rows = self._fetchall(
                """
                SELECT r.run_id, r.conversation_id, r.status, r.scope_json, r.incident_id,
...
def _insert_event(
    ...
    cursor = conn.execute(
        ...
        """,
        (run_id, event_type, thread_type, redact(message), _json_dumps(redact(payload)), created_at, actor_id, promoted_from_event_id),
    )

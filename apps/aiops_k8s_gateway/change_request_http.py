...
def dispatch(
    ...
    if create_incident_id is None and input_request_id is None and detail_request_id is None:
        return False
    ...
    if handler.command == "GET" and detail_request_id is not None:
        ...
        return True
    if handler.command != "POST" or (create_incident_id is None and input_request_id is None):
        return False
...
def send_plan_request(payload: dict[str, object]) -> dict[str, object]:
...
def _detail_route(path: str) -> str | None:
    prefix = "/api/v1/change-requests/"
    if not path.startswith(prefix) or path.endswith("/input"):
        return None

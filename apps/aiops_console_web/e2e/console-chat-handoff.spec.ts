...
test("AI 对话固定端到端验收流程", async ({page}, testInfo) => {
  ...
  await expect(page.getByRole("combobox", {name: "转交目标类型"})).not.toBeVisible()
  await page.getByRole("button", {name: "转交事件调查"}).click()
  await expect(page.getByText("核对转交内容")).toBeVisible()
  await page.waitForTimeout(250)
  await page.screenshot({path: testInfo.outputPath("chat-handoff-dialog.png"), fullPage: true})

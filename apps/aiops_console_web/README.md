...
提升前，workflow 从当前 Deployment 的 immutable Console 与 Gateway digest 分别读取 monorepo revision，并用已部署 Gateway contract 构建候选 Console、用候选 Gateway contract 构建上一 Console。不兼容变更必须使用新的 API version；首次部署没有上一 Console artifact 时不执行 N-1 检查。

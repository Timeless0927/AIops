...
export function ResourceWorkspaceView({data, filters, onChange}: {
  ...
  return <main className="mx-auto w-full max-w-[1600px] min-w-0 px-3 py-5 sm:px-4 lg:px-6">
    <PageHeader title="资源" description="Cluster、Service 与 Deployment Target">
    ...
    <section aria-label="资源筛选" className="flex min-w-0 flex-wrap gap-2 py-3">
    ...
    {resources.length ? <ul className="mt-2 grid gap-2" aria-label="资源列表">{resources.map((resource) => {
      ...
      return <li key={resource.id}><Link
        ...
        className={filters.resource === resource.id
          ? "grid min-w-0 gap-2 rounded-lg border border-evidence/40 bg-accent/50 px-3 py-3 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring sm:grid-cols-[minmax(0,1.4fr)_minmax(12rem,0.8fr)_auto] sm:items-center"
          : "grid min-w-0 gap-2 rounded-lg border bg-card/50 px-3 py-3 outline-none transition-colors hover:bg-accent/40 focus-visible:ring-2 focus-visible:ring-ring sm:grid-cols-[minmax(0,1.4fr)_minmax(12rem,0.8fr)_auto] sm:items-center"}
      >
    ...
    })}</ul> : <Empty className="min-h-64 border rounded-lg"><EmptyHeader><EmptyMedia variant="icon"><BoxesIcon /></EmptyMedia><EmptyTitle>没有匹配的资源</EmptyTitle><EmptyDescription>当前筛选范围内没有 Resource。</EmptyDescription></EmptyHeader></Empty>}
  </main>
}

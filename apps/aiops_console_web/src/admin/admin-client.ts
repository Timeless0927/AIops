...
export function getModelProviderDetail() {
  return request<components["schemas"]["ModelProviderDetailResponse"]>("/api/v1/admin/model-provider")
    .then((response) => response.model_provider)
}

export function getMCPIntegrations() {
  return request<components["schemas"]["MCPIntegrationListResponse"]>("/api/v1/admin/mcp-integrations")
    .then((response) => response.mcp_integrations)
}

export function createMCPIntegration(body: MCPIntegrationCreate) {
  return write<components["schemas"]["MCPIntegrationResponse"]>("/api/v1/admin/mcp-integrations", "POST", body)
    .then((response) => response.mcp_integration)
}

export function updateMCPIntegration(id: string, body: MCPIntegrationUpdate) {
  return write<components["schemas"]["MCPIntegrationResponse"]>(`/api/v1/admin/mcp-integrations/${encodeURIComponent(id)}`, "PATCH", body)
    .then((response) => response.mcp_integration)
}

export function verifyMCPIntegration(id: string, reason: string) {
  return write<components["schemas"]["MCPIntegrationResponse"]>(`/api/v1/admin/mcp-integrations/${encodeURIComponent(id)}/verify`, "POST", {reason})
    .then((response) => response.mcp_integration)
}

export function getSkills() {
  return request<components["schemas"]["SkillListResponse"]>("/api/v1/admin/skills").then((response) => response.skills)
}

export function createSkill(body: SkillCreate) {
  return write<components["schemas"]["SkillResponse"]>("/api/v1/admin/skills", "POST", body).then((response) => response.skill)
}

export function createSkillVersion(id: string, body: SkillVersionCreate) {
  return write<components["schemas"]["SkillResponse"]>(`/api/v1/admin/skills/${encodeURIComponent(id)}/versions`, "POST", body)
    .then((response) => response.skill)
}

export function enableSkill(id: string, version: number, expectedActiveVersion: number | null, reason: string) {
...
export function saveModelProvider(body: ModelProviderSave) {
  return write<components["schemas"]["ModelProviderDetailResponse"]>("/api/v1/admin/model-provider", "PUT", body)
    .then((response) => response.model_provider)
}
...
export function getKubernetesChangeAuthorities() {
  return request<components["schemas"]["KubernetesChangeAuthorityListResponse"]>("/api/v1/admin/kubernetes-change-authorities")
    .then((response) => response.kubernetes_change_authorities)
}

export function createKubernetesChangeAuthority(body: components["schemas"]["KubernetesChangeAuthorityCreateRequest"]) {
...
export function updateKubernetesChangeAuthority(id: string, body: components["schemas"]["KubernetesChangeAuthorityUpdateRequest"]) {
  return write<components["schemas"]["KubernetesChangeAuthorityResponse"]>(

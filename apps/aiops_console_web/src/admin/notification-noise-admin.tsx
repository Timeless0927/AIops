...
export function NotificationNoiseAdmin({reason}: {reason: string}) {
  ...
  return <section className="flex flex-col gap-5 border-t pt-6" aria-labelledby="noise-heading">
    ...
    {selected ? <form key={selected.id} className="grid gap-3 lg:grid-cols-[220px_1fr_1fr_140px_140px_auto]" onSubmit={(event) => {
      ...
      mutation.mutate(() => updateNotificationNoiseControl(selected.id, {
      ...
      <div className="flex items-end"><Button type="submit" disabled={!reason || mutation.isPending}><SaveIcon />保存</Button></div>
    </form> : <div className="text-sm text-muted-foreground">暂无 Destination</div>}

    <div className="border-t pt-5">
      <form className="grid gap-3 lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_220px_auto]" onSubmit={(event) => {
        event.preventDefault()
        const form = new FormData(event.currentTarget)
        ...
        mutation.mutate(() => createNotificationSilence({match, expires_at: new Date(String(form.get("expires_at"))).getTime() / 1000, reason}))
      }}>
        ...
        <div className="flex items-end"><Button type="submit" variant="destructive" disabled={!reason || mutation.isPending}><BellOffIcon />创建 Silence</Button></div>
      </form>
    </div>

...
function sampleRequest(eventType: string) {
  const status = eventType.split(".")[1]
  const facts = eventType === "incident.severity_changed" ? {incident_id: "preview", status, previous_severity: "warning", severity: "critical"} : eventType.startsWith("incident.") ? {incident_id: "preview", status} : eventType.startsWith("investigation.") ? {incident_id: "preview", investigation_id: "preview", status, reason: "Preview"} : eventType === "approval.required" ? {incident_id: "preview", investigation_id: "preview", action_id: "preview", status} : eventType === "approval.blocked" ? {incident_id: "preview", investigation_id: "preview", action_id: "preview", status, reason: "Preview"} : eventType.startsWith("approval.") ? {incident_id: "preview", investigation_id: "preview", action_id: "preview", approval_id: "preview", status} : eventType.startsWith("execution.") ? {incident_id: "preview", command_id: "preview", action: "restart", status} : {connector_id: "preview", cluster_id: "preview", status}
  return {event_id: `preview:${crypto.randomUUID()}`, event_type: eventType, occurred_at: Date.now() / 1000, severity: "critical", subject: {type: eventType.split(".")[0], id: "preview", version: 1}, scope: {environment: "prod", team_id: "preview", service_id: "preview"}, summary: "Checkout unavailable", facts, console_path: "/incidents/preview"}
}

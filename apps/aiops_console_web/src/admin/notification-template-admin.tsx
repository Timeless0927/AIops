...
export function NotificationTemplateAdmin() {
  ...
  return <section className="flex flex-col gap-4 border-t pt-6" aria-labelledby="template-heading">
    ...
    <Accordion><AccordionItem value="copy-notification-template">
      <AccordionTrigger><span><span className="block font-medium">复制内置模板</span><span className="mt-1 block text-xs font-normal text-muted-foreground">从 provider 与 event 的内置模板创建可编辑副本</span></span></AccordionTrigger>
      <AccordionContent><form className="grid gap-3 md:grid-cols-[180px_1fr_1fr_auto]" onSubmit={(event) => {
      ...
      <div className="flex items-end"><Button type="submit" disabled={mutation.isPending}><CopyIcon />复制内置模板</Button></div>
    ...
    {selected ? <Accordion defaultValue={[`edit-${selected.id}`]}><AccordionItem value={`edit-${selected.id}`}>
      <AccordionTrigger><span><span className="block font-medium">编辑 {selected.name}</span><span className="mt-1 block text-xs font-normal text-muted-foreground">保存新版本后重新预览和测试</span></span></AccordionTrigger>
      <AccordionContent><form key={`${selected.id}:${selected.version}`} className="grid gap-3 md:grid-cols-2" onSubmit={(event) => {
      event.preventDefault(); const form = new FormData(event.currentTarget)
      ...
      <div className="flex flex-wrap gap-2 md:col-span-2">
        <Button type="submit" disabled={mutation.isPending}>保存新版本</Button>

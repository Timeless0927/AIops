...
function simulationFacts(event: string) { const status = event.split(".")[1]; if (event === "incident.severity_changed") return {incident_id: "simulation", status, previous_severity: "low", severity: "high"}; if (event.startsWith("incident.")) return {incident_id: "simulation", status}; if (event.startsWith("investigation.")) return {incident_id: "simulation", investigation_id: "simulation", status, reason: "simulation"}; if (event === "approval.required") return {incident_id: "simulation", investigation_id: "simulation", action_id: "simulation", status}; if (event === "approval.blocked") return {incident_id: "simulation", investigation_id: "simulation", action_id: "simulation", status, reason: "simulation"}; if (event.startsWith("approval.")) return {incident_id: "simulation", investigation_id: "simulation", action_id: "simulation", approval_id: "simulation", status}; if (event.startsWith("execution.")) return {incident_id: "simulation", command_id: "simulation", action: "simulation", status}; return {connector_id: "simulation", cluster_id: "simulation", status} }

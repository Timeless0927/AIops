...
门禁记录（T06）：Resource Catalog Module 为 `apps/aiops_k8s_gateway/resource_catalog.py`，公开 Interface 是 discovery refresh、Service 创建、Binding 确认/纠正和 execution target guard，定向 selector 为 `tests/test_gateway_resource_catalog.py` 与 `tests/test_gateway_v1_resource_catalog_contract.py`。Connector discovery Module 为 `apps/cluster_connector/discovery.py`，Gateway client Interface 为 `apps/cluster_connector/gateway_client.py`，定向 selector 为 `tests/test_connector_discovery.py` 与 `tests/test_connector_registration_recovery.py`。本票触碰的超大旧文件 `main.py` 从 5517 行降至不高于起始行数，`v1_store.py` 从 1047 行下降；其中新增的 `authenticate_connector` 仅暴露 T05 Connector identity 校验，不拥有 T06 状态。`toolsets/topology_store.py` 所属 Kubernetes Inventory Module 的公开 Interface 为 `KubernetesInventory`，定向 selector 为 `tests/test_topology_store.py`。

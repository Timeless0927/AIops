...
门禁记录（T23）：Connector Journal Module 为 `apps/cluster_connector/command_worker.py`，公开 Interface 是 durable journal、execution lock 与固定 cleanup，定向 selector 为 `tests/test_connector_command_worker.py`。Diagnosis Job Module 为 `diagnosis_service/jobs.py`，公开 Interface 是 durable execution/writeback 与 retained terminal cleanup，定向 selector 为 `tests/test_diagnosis_jobs.py`。Notification Delivery Module 为 `notification_service/requests.py`（任务开始 658 行，完成 713 行），公开 Interface 是 durable Request/Delivery lifecycle 与 fixed terminal cleanup，定向 selector 为 `tests/test_notification_service.py`；该文件保持单一 Delivery owner 且低于 800 行。Gateway Connector Command Module 为 `apps/aiops_k8s_gateway/connector_commands.py`（任务开始 642 行，完成 673 行），公开 Interface 是 Command Lease reconciliation，定向 selector 为 `tests/test_gateway_connector_commands.py`。Kubernetes deployment contract 测试为 `tests/test_k8s_manifests.py`（任务开始 648 行，完成 668 行），只验证单副本与独立 PVC。Connector、Diagnosis 与 Notification worker 启动即 cleanup，之后按固定一小时节流；每次最多清理 1000 个 owner record，并通过 bounded-label eligible backlog metric 观察。Connector DB 增加显式 forward migration 与 execution lock foreign key，旧 journal 可原地升级；Unknown Outcome 保留产品状态但清理过期技术 lease，仍接受 exact lease identity 的晚到结果。受影响 Module 与直接 contract 共 120 passed，Python 语法编译和 diff check 通过；全量 pytest 的 9 项失败与 T22 基线一致，均属于冻结 legacy Connector registration/diagnosis 路径。

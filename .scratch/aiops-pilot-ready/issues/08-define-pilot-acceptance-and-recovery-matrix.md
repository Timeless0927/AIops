...
| `I01` | Operator 只执行 `kubectl apply -k ./aiops-pilot-vX.Y.Z`。API Server 接受完整 manifest，bootstrap 2m 内 Complete，6 个 PVC 10m 内 Bound，全部 release Deployment 10m 内 Available。 | apply output、object UID/image digest、Job/PVC/Deployment conditions、events | 任一失败阻塞；保留 describe/log，不用手工 patch 绕过。 |

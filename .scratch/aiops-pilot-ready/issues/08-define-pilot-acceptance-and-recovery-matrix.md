...
`S04` 的真实回执 attestation 同时验证本次 acceptance 使用的 exact Destination revision。S04 `advance` 在真实 test Delivery terminal 后持久化 receipt 并保持 gate open，独立 `attest` 绑定 exact receipt SHA，随后 `resume` 才启用 Destination/选择 Route；等待签名不得记为失败，resume 不得重发 test Delivery。`V07`/`V08` 继续使用该 revision 时，以可关联的 terminal `sent` Delivery 作为送达证据，不重复要求人员确认；Destination revision 变化后必须先取得新的真实回执 attestation。
...
| `S04` | 管理员创建无效 Notification configuration，真实 test Delivery bounded retry 后 `dead_letter`；修复后创建新 revision，真实 test Delivery 在 15m 窗口内 `sent`。`advance` 保存 receipt 并保持 open；管理员实际看到消息后独立签名，`resume` 不重发 Delivery，只启用 exact revision 并选择 Pilot catch-all Route。 | Delivery/attempt/provider identity、安全 reason、Route revision、Platform Status、绑定 receipt SHA 的 HITL attestation | false sent、缺 provider identity、未实际送达、resume 重发、未 selected 或最终未 ready 阻塞。预期 dead-letter 是 pass。 |

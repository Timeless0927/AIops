...
实际发布物为 `dist/aiops-pilot-v0.1.0.tar.gz`（41 KiB）与 `dist/SHA256SUMS`。A01 live acceptance 暴露并修复 Platform Status、Notification、Model readiness、remote HTTP client ID、Connector Pod JSON、command-loop Interface/readiness 与 telemetry probe source 问题后，已用公开可匿名拉取的 Console `9d16af2e…c9102`、Gateway `c8c53d20…5b2bcd`、Connector `a0ed84d4…581c45`、Diagnosis `1d3134f0…fe18f` 与 Notification `1a73d5d3…a50444` digest 重建当前 candidate，archive SHA256 为 `70bf805e02be8b8f9ef9a5d00d3779921aaf91f97d87e7b64725a60a3c7792be`；此前 `59880499…42a`、`fc1b649d…c11a`、`43ca9d4a…13935`、`f0e10b04…0418`、`c9c0e106…6600`、`4cf9a808…8efa`、`7e2b22c3…58bc4`、`fdd43028…51490a` 与 `e8f28d2f…fc0122` artifact 均已被 supersede，不得用于 A01。空目录测试验证 safe extraction、checksum、确定性 rebuild、唯一顶层 overlay、本地 render、完整 file/image inventory、zero/tag/Operator CRD 禁止、verification dry-run 与 OpenAPI 全量重新生成逐字节一致；原 package selector 7 passed，Pilot/verification/contract 直接消费者合计 68 passed，本次 candidate 重建后发布/交付 selector 63 passed，TypeScript/Vite production build 通过，仅保留既有 >500 kB warning。归档与 metadata 只声明 clean install/same-version reapply，并明确排除 upgrade、downgrade、backup、rollback 与 data-preserving uninstall；Standards 与 Spec 双轴复审均零 finding。
...
当前 live candidate（A01）：`dist/aiops-pilot-v0.1.0.tar.gz` SHA256 `70bf805e02be8b8f9ef9a5d00d3779921aaf91f97d87e7b64725a60a3c7792be`；Console `sha256:9d16af2e33d63766f57239e64779338f4f8ec0f1c246ae6d355b9ea06c0c9102`、Gateway `sha256:c8c53d20942b4c432b3aa0fdeb47bb219ff987b6f0a44ede92f8e5f4925b2bcd`、Connector `sha256:a0ed84d429d7dce916e0ff9aeb215e832dacfcfd14454e937f9f8993fa581c45`、Diagnosis `sha256:1d3134f08e00bbb29d3366041b56c6b54299b31824c618cdeaf1f7ae7d4fe18f` 与 Notification `sha256:1a73d5d3bf8e2f95b4d2c1f229657765b78628dc2e98c68b6d405af545a50444` 均已通过 split-image import smoke、推送和空 Docker config 匿名 manifest inspect；Connector public image smoke 启动真实后台 target，经公开 `/readyz` 验证 running 为 200，cleanup 后同一公开 Interface 为 503。先前 `f0e10b04…0418`、`c9c0e106…6600`、`4cf9a808…8efa`、`7e2b22c3…58bc4`、`fdd43028…51490a` 与 `e8f28d2f…fc0122` candidate 已被 supersede，不得继续用于 A01。builder 直连 `registry.k8s.io` 超时后，仅重建进程对该 registry 使用用户授权代理，Aliyun/Quay/DaoCloud 保持直连；代理未写入项目或 Cluster 配置。

...
门禁记录（K01）：Change Request/Plan Module 为 `apps/aiops_k8s_gateway/change_requests.py`（501 行），公开 Interface 是提交 Change Request、追加 blocking input、读取 actor-scoped projection；它独占 request、active Phase、immutable revision/input/event 的 `gateway.db` 状态，Change Request status 只投影 active Phase。Diagnosis model Module 为 `diagnosis_service/change_planner.py`，内部 HTTP Adapter 为 `change_planner_http.py`；只接收 Gateway 提供的 sanitized facts，不持有 Kubernetes credential，也不持久化 model reasoning。Gateway/Console Adapter 为 `change_request_http.py`、generated OpenAPI types 与 `workbench-page.tsx`；后者从任务开始 480 行增至 580 行，仍只承载 Incident Workbench 页面职责。定向 selector 为 `tests/test_gateway_v1_change_requests_contract.py`、`tests/test_change_planner.py`、`tests/test_diagnosis_service.py::test_post_change_plan_uses_authenticated_model_boundary` 与 `apps/aiops_console_web/src/api/client.test.ts`；直接 contract/architecture 共 38 passed，Console Vitest 6 passed、TypeScript no-emit 与 production build 通过，最终全量 pytest 为 485 passed。新文件均低于 800 行，`diagnosis_service/service_main.py` 保持 499 行且只新增 route 装配。

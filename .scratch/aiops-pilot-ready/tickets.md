...
实际发布物为 `dist/aiops-pilot-v0.1.0.tar.gz`（41 KiB）与 `dist/SHA256SUMS`。A01 live acceptance 暴露并修复 Platform Status、Notification、Model readiness、remote HTTP client ID、Connector Pod JSON、command-loop Interface/readiness 与 telemetry probe source 问题后，已用公开可匿名拉取的 Console `9d16af2e…c9102`、Gateway `c8c53d20…5b2bcd`、Connector `8b237e25…30e2ce`、Diagnosis `1d3134f0…fe18f` 与 Notification `1a73d5d3…a50444` digest 重建当前 candidate，archive SHA256 为 `9528a033d18dbb23220375a258e6b839f32270171d0550c4eb97b3c1ef81ef36`；此前 `59880499…42a`、`fc1b649d…c11a`、`43ca9d4a…13935`、`f0e10b04…0418`、`c9c0e106…6600`、`4cf9a808…8efa`、`7e2b22c3…58bc4`、`fdd43028…51490a`、`e8f28d2f…fc0122` 与 `70bf805e…c7792be` artifact 均已被 supersede，不得用于 A01。空目录测试验证 safe extraction、checksum、确定性 rebuild、唯一顶层 overlay、本地 render、完整 file/image inventory、zero/tag/Operator CRD 禁止、verification dry-run 与 OpenAPI 全量重新生成逐字节一致；原 package selector 7 passed，Pilot/verification/contract 直接消费者合计 68 passed，本次 candidate 重建后发布/交付 selector 63 passed，TypeScript/Vite production build 通过，仅保留既有 >500 kB warning。归档与 metadata 只声明 clean install/same-version reapply，并明确排除 upgrade、downgrade、backup、rollback 与 data-preserving uninstall；Standards 与 Spec 双轴复审均零 finding。
...
完成记录（A01 runner）：Acceptance Evidence Module 的公开 Interface 为 immutable candidate/run identity、frontier/append-only gate attempt、manifest/artifact revalidation、脱敏 artifact/hash、OpenSSH-signed human attestation 与最终 `SHA256SUMS`；Package/Cluster/Web/Platform Status/Model/Notification/Connector/Observability runner 只编排 subprocess、Kubernetes、Gateway HTTP、真实 Playwright 和 evidence，不写产品 DB 或第二套 acceptance state。HTTP client 显式禁用环境代理，浏览器使用 `--no-proxy-server`，通用 subprocess Adapter 剥离 ambient proxy environment；外部 credential 只经内存/getpass/子进程 stdin 流转。`P01-P03`、`I01-I05`、`S01-S06` policy、clean preflight 32Gi PVC/全节点 digest pull、same-version reapply Secret hash、role/CSRF/fresh-auth、invalid→real provider、dead-letter→sent/selected、release-bound Connector identity/Secret/rollout/read verification 和真实 `aiops_*` telemetry/MCP Adapter 均有定向测试。`evidence.py`（589 行）的单一职责是 fail-closed acceptance ledger，公开 Interface 为 create/open/start/record/attest/finalize；`cluster_install.py`（798 行）的单一职责是 clean Cluster preflight 与同一 release install/reapply Kubernetes gate，公开 Interface 为 `run_p03/run_i01/run_i02`；二者低于 800 行且各自通过公开 Interface 测试，不为行数拆出转发层。Model、Notification、Connector、Observability 与 Platform Status gate 已按独立变化能力拆为各自低于 500 行的 Module。Notification Delivery owner `notification_service/requests.py` 为 783 行，公开 Delivery history Interface 只补回 durable attempt ID 以满足既定 retained evidence contract，保持低于 800 行；定向 selector 为 `tests/test_notification_service.py` 与 `tests/test_gateway_v1_notification_contract.py`。Gateway `main.py` 仍为 788 行，A01 只把现有 Platform Status Adapter 装配到既有 fresh-auth guard；领域判断留在 Adapter。A01 定向 selector 为 `tests/test_pilot_acceptance_*.py`、`tests/test_gateway_v1_platform_status_contract.py`、`tests/test_platform_status.py` 及前述 Notification selectors。最终 live acceptance 记录见下。

实现验收（A01 runner）：最终定向 selector 73 passed；真实 P01/P02 runner 均 passed，其中 P02 当前源代码 contract 82 passed、Console 17 passed且 production build 通过；全量 pytest 735 passed/4 skipped；Standards 与 Spec 双轴最终复审均零 finding。Platform Status authenticated SSE 复用真实 owner snapshot，使 clean install 无需 seed Incident 即可完成 I03 handshake。仓库测试只证明 runner/product contract；A01 的真实通过结论单独由下方最终 clean run 证明。

当前 live candidate（A01）：`dist/aiops-pilot-v0.1.0.tar.gz` SHA256 `9528a033d18dbb23220375a258e6b839f32270171d0550c4eb97b3c1ef81ef36`；Console `sha256:9d16af2e33d63766f57239e64779338f4f8ec0f1c246ae6d355b9ea06c0c9102`、Gateway `sha256:c8c53d20942b4c432b3aa0fdeb47bb219ff987b6f0a44ede92f8e5f4925b2bcd`、Connector `sha256:8b237e25fffe63aeab10cd4ce209ef9826a523707101d7fdb2719cb6f430e2ce`、Diagnosis `sha256:1d3134f08e00bbb29d3366041b56c6b54299b31824c618cdeaf1f7ae7d4fe18f` 与 Notification `sha256:1a73d5d3bf8e2f95b4d2c1f229657765b78628dc2e98c68b6d405af545a50444` 均已通过 split-image import smoke、推送和空 Docker config 匿名 manifest inspect；Connector public image smoke 启动真实后台 target并经公开 `/readyz` 验证 running 为 200，仓库回归在 cleanup 后经同一公开 Interface 验证 503。先前 `f0e10b04…0418`、`c9c0e106…6600`、`4cf9a808…8efa`、`7e2b22c3…58bc4`、`fdd43028…51490a`、`e8f28d2f…fc0122` 与 `70bf805e…c7792be` candidate 已被 supersede，不得继续用于 A01。builder 直连 `registry.k8s.io` 超时后，仅重建进程对该 registry 使用用户授权代理，Aliyun/Quay/DaoCloud 保持直连；代理未写入项目或 Cluster 配置。

Live 进度（2026-07-15）：用户授权清空既有 Pilot 后，canonical overlay 已删除 `aiops-system` 与全部 release Cluster RBAC，`aiops-verification` 原本不存在；NodePort 30088 空闲，默认 `local-path` StorageClass 与两节点 Ready Calico 已核实。首个正式 evidence `/root/aiops/acceptance/v0.1.0-20260714T235407Z` 绑定当时的 archive SHA `fc1b649d32d74c1037dde99b7065f29f0a615d14b1cb7fd577b01a3f9822c11a` 及当前 kube context/cluster identity，P01/P02 连续 passed；其后 P03 attestation 与失败 attempt 见下条记录，该旧 evidence 不属于当前 candidate。
...
后续失败 run（证据保留）：`/root/aiops/acceptance/v0.1.0-20260715T020132Z` 绑定 archive SHA `43ca9d4a64772c4db33a732da64769b436fb77e8df87413741b2879e66313935`，从 clean Cluster 连续完成 P01-P03/I01-I05/S01；ordinary User 再次只通过 Console 创建，独立浏览器登录并确认 admin API 403。S02 等待真实 301 秒 fresh-auth 窗口后，model/notification configure 两项未产生预期 stale audit；根因是 acceptance probe 对这两条 configure route 使用空 body，先被 Adapter 字段校验拒绝，未到达 freshness seam，并非产品拒绝路径漏审计。runner 改用结构合法、明确不会发送的非凭据占位 body，fake Session 同步真实 Adapter 的 validation→freshness 顺序；定向 consumer 6 passed且双轴复审零 finding。该失败 run 不再继续，修复后仍从 clean P01 重来。

前一连续成功 run：`/root/aiops/acceptance/v0.1.0-20260715T021825Z` 同样绑定 archive SHA `43ca9d4a64772c4db33a732da64769b436fb77e8df87413741b2879e66313935`，已从 clean Cluster 完成 P01-P03/I01-I03/I05/S01-S02，HTTP profile 的 I04 正确记为 not applicable；Console UI 创建的新 ordinary User 在独立浏览器登录成功且 admin API 返回 403，credential 只存于 workspace 外 0600 文件。S02 以真实 301 秒窗口证明六类 CSRF 与六类 stale mutation guard 均返回 403 并保留 exact admin audit correlation。S03 尚未开始：本机没有真实 OpenAI-compatible Provider credential、本地模型服务或可复用配置；mandatory 两轮 tool-use/nonce probe 不得以 mock 或伪造 credential 替代。后续 S04 同样需要真实 Notification destination credential 与人员收件确认。
...
前一 clean run：`/root/aiops/acceptance/v0.1.0-20260715T034203Z` 绑定 archive SHA `c9c0e1063afb4d3a61a08d50b8207b30a0988a87426fae8416c6aa5005fb6600`，已从精确删除 `aiops-system`、`aiops-verification` 与五组 release Cluster RBAC 后连续完成 P01-P03/I01-I03/I05/S01-S03，I04 按 HTTP NodePort profile 记为 not applicable。新 Console digest 的真实无头 Chromium admin 登录 200、用户创建 POST 201，ordinary User 登录、角色边界与 CSRF 浏览器矩阵通过；S02 再次以真实 301 秒窗口验证 stale mutation guard；S03 使用 workspace 外 0600 DeepSeek 输入完成 invalid→verified 两轮 tool-use/bare-JSON/nonce probe。该 run 的 S04 后续失败，不属于当前 candidate。

S04 首次 attempt（失败证据保留）：用户通过修复后的 Web 配置真实 DingTalk Destination `dd`，owner 投影为 `ready`；runner 从 Notification owner 在内存读取加密配置且未输出 plaintext，但创建故障 Destination 时返回 `400 notification_configuration_rejected`。根因是 Feishu/DingTalk invalid fixture 使用 `127.0.0.1`，在真实投递前已被官方域名 trust-boundary validation 拒绝；fixture 已改为官方域名上的 acceptance-only invalid token，使请求可进入真实 Delivery path。新增 owner 公开 Interface 回归先红后绿，A01 Integration/Notification owner/Gateway 直接消费者 29 passed；该 run 不再继续，修复后从 clean P01 重来。

后续 clean run（失败证据保留）：`/root/aiops/acceptance/v0.1.0-20260715T043749Z` 同样绑定 `c9c0e106…6600`，连续完成 P01-P03/I01-I03/I05/S01-S03 后，S04 故障 DingTalk Delivery 反而以一次 attempt 记为 `sent`。真实 owner DB 证明非零错误 envelope 被错误接受；根因是 DingTalk/Feishu 以 HTTP 200 承载业务错误，而 Apprise 只判断 HTTP status，Notification Adapter 未检查非零 `errcode`/`code`。runner 在 15m 空转前被中断且未写 S04 gate result，该 run 不再继续。Adapter 现只对 Apprise 已判成功的 HTTP 200 检查 Provider envelope：非零值映射 non-retryable `provider_rejected`，零值仍成功，429/5xx/Retry-After 与 SMTP 语义保持；回归先红后绿，Notification/acceptance 直接消费者 60 passed，双轴复审零 finding。修复镜像已发布为上述 Notification digest，并据此重建当前 candidate。

当前 candidate 首次 clean run（失败证据保留）：`/root/aiops/acceptance/v0.1.0-20260715T051255Z` 绑定 `4cf9a808…8efa`，连续完成 P01-P03/I01-I03/I05/S01-S03；新 Notification Adapter 已令故障 DingTalk Delivery 以一次 non-retryable durable attempt 达到 `dead_letter/provider_rejected`，但 runner 随后在保存真实 config 时同时请求 `enabled: true`，owner 因该新 revision 尚未 test `sent` 返回 `400 notification_configuration_rejected`。修复保持 owner guard 不变，将顺序改为 disabled save→test sent→HITL receipt→enable exact revision→select route；integration fake 现拒绝未验证启用并要求 sent 后 activation，回归先红后绿，A01/Notification 直接消费者 64 passed。该失败 attempt 不覆盖，runner 修复后仍从 clean P01 重来；产品 candidate 不变。

最新 clean run（S05 未完成）：`/root/aiops/acceptance/v0.1.0-20260715T053029Z` 绑定 `4cf9a8080482473ea500aa42db13bc0c89bb6a553ee1b81ea63ae02b93bc8efa`，已连续完成 P01-P03/I01-I03/I05/S01-S04，I04 为 not applicable；S04 故障 DingTalk test 真实 `dead_letter/provider_rejected`、真实 revision test `sent`，用户按 exact Delivery ID 确认群消息后完成签名、enable 与 Pilot Route select。S05 Enrollment 随后达到 `registered/online`，但 read verification 为 `invalid_verification_response`：真实 Connector Command `succeeded/exit_code=0/truncated=false`，其 Pod JSON 被通用行级 redactor 将 `serviceAccountToken` schema 改成非法 `[REDACTED]`，且真实 kubectl collection 使用 `kind: List` 而 Gateway 只接受 `PodList`。runner 在 10m 空转前中断且未写 S05 gate result，该 run 不再继续。修复将合法 JSON 改为结构化敏感键/env/Secret-ref 脱敏并保留非凭据 schema，Secret data 仍全量遮蔽、ConfigMap data 不过度遮蔽；Gateway 对 `List`/`PodList` 统一验证同 apiVersion Pod items并规范化为 `PodList` discovery。Connector Enrollment owner 现 770 行，公开 Interface 不变，定向 selector 为 `tests/test_connector_enrollments.py` 与 `tests/test_gateway_v1_connectors_contract.py`；Redaction Module 228 行，定向 selector 为 `tests/test_k8s_redact.py`。相关 selector 16+48 passed，真实 kubectl JSON replay `kind=List/pods=2` 通过，双轴复审零 finding；须发布新 Gateway/Connector digest、重建 candidate 并再次 clean P01。

新 candidate 发布验证（2026-07-15）：Gateway `sha256:c8c53d20942b4c432b3aa0fdeb47bb219ff987b6f0a44ede92f8e5f4925b2bcd` 与 Connector `sha256:188b3bf6be80588d109348086067cb6d889cef8f183aac569bf2402b16bb834c` 直连推送成功；两个 digest 均以空 Docker config 完成匿名 manifest inspect、按 digest 拉取和 split-service import smoke。builder 直连 `registry.k8s.io` 的匿名 inspect 首次 60s 超时，仅本次重建进程对该 registry 使用 `10.0.41.206:30789`，其它 registry 置于 `NO_PROXY`；新归档 SHA256 为 `7e2b22c32c5706b5e815b38b4a626b5b938d7ebcac0bd7b6c7de814598458bc4`。发布/交付定向测试 63 passed；该 candidate 尚未开始新的 live acceptance，必须从 clean P01 重跑。

后续 clean run（S05 未完成）：`/root/aiops/acceptance/v0.1.0-20260715T061830Z` 绑定 `7e2b22c32c5706b5e815b38b4a626b5b938d7ebcac0bd7b6c7de814598458bc4`，连续完成 P01-P03/I01-I03/I05/S01-S04，I04 为 not applicable；S04 新钉钉消息 `notification-delivery:acceptance-s04-real-test`、Time `1784097896` 获用户真实确认并完成签名。S05 rollout 到 Connector `188b3bf6…bb834c` 后，command-poll 线程立即以 `run_command_cycle() got an unexpected keyword argument 'mutation_executor'` 崩溃；runner 已中断且未写 S05 gate result，该 run 不再继续。根因是 legacy mutation contract 退役时 worker Interface 已改为 validation/change/reconciliation executor，但进程装配仍传入已删除参数；最小修复删除该过期注入与未使用 import，不新增兼容层。0.11 秒 command-loop Interface 回归先红后绿，Connector/Gateway/redaction 直接消费者 66 passed/2 skipped；新 Connector `sha256:07ffbb1dd0f1f308f22593fd344a365ee282f8fbfe98f2114aebcb1273aafc6b` 已完成镜像内 Interface/import smoke、直连推送、空 Docker config 匿名 inspect 与按 digest 拉取。新归档 SHA256 为 `fdd4302867bebd657eda2aa9d246c1aa0e4becc92facb3e0fbf31c7e0751490a`，发布/交付定向测试 63 passed；必须再次 clean P01。

最新 clean run（S06 失败证据保留）：`/root/aiops/acceptance/v0.1.0-20260715T071217Z` 绑定 `fdd4302867bebd657eda2aa9d246c1aa0e4becc92facb3e0fbf31c7e0751490a`，连续完成 P01-P03/I01-I03/I05/S01-S05，I04 为 not applicable；S04 新钉钉消息 Time `1784100436` 获用户真实确认并签名，S05 使用修复后的 Connector 完成 `online/verified` Pod read verification，用户确认 one-time credential handling 后签名。S06 首个 raw Prometheus query 从 Diagnosis Pod 访问 `aiops-prometheus:9090` 持续超时并记为 failed，该 run 不再继续。Service/Endpoints 与 Prometheus runtime 正常，MCP-Prometheus→Prometheus 立即 Ready；根因是 acceptance telemetry Adapter 错把 Diagnosis 当成 raw backend curl 跳板，而 canonical NetworkPolicy 正确地只允许 MCP/Gateway 访问 observability backend。修复不放宽进程边界：Prometheus raw query 从 MCP-Prometheus Pod、Loki raw query 从 MCP-Loki Pod、Alertmanager runtime config 从其 Pod 内置 `amtool` 查询，MCP guarded calls 仍从 Diagnosis 发起。Adapter 来源路由回归先红后绿，相关 selector 16 passed/2 skipped；原 live probe 1.13 秒通过，13 个 targets up、rules healthy、134 条 `aiops_*` series、9 个 Loki streams、Alertmanager route true、Prometheus/Loki MCP 均 succeeded。同期 Connector readiness 改为 command polling thread 非存活即 `/readyz` 503；首次 image smoke 的 fake `is_alive()` 可在线程退出后假绿，终审发现后改为真实后台 target + public `/readyz` running 200，仓库 lifecycle 回归在 cleanup 后验证 503。最终 Connector `8b237e25…30e2ce` 与归档 `9528a033…81ef36` 已发布/验证，必须再次 clean P01。

最终 clean run（A01 passed，2026-07-15）：`/root/aiops/acceptance/v0.1.0-20260715T082255Z` 从 clean Cluster 连续绑定最终 archive SHA `9528a033d18dbb23220375a258e6b839f32270171d0550c4eb97b3c1ef81ef36`，P01-P03、I01-I03/I05 与 S01-S06 均一次 passed，HTTP NodePort profile 的 I04 正确记为 not applicable，任何 gate 均无失败 attempt。S03 完成真实 DeepSeek invalid→verified 两轮 tool-use/nonce probe；S04 完成真实 DingTalk `dead_letter/provider_rejected`→`sent`、用户收件确认与 Pilot Route select；S05 完成 Connector `online/verified` Pod read verification及 one-time credential handling 人员确认；S06 证明 13 个 targets up、rules loaded、135 条真实 `aiops_*` series、10 个 Loki streams、Alertmanager route 与 Prometheus/Loki MCP 均 succeeded。`finalize` 对 P03/I05/S04/S05 四份 OpenSSH attestation 重新验签，最终 `manifest.json` 为 `promotion_eligible: true`，56 项 `SHA256SUMS` 全部复算一致；archive、manifest 与 human attestation SHA256 分别为 `9528a033d18dbb23220375a258e6b839f32270171d0550c4eb97b3c1ef81ef36`、`28a5ee9ba6a4da1ff15be2f8d59ca5fe1ecab501433c5959c1e3ba0be16f769a` 与 `6654d5a3932d09a58ea0fc7b563b9ad52ebbf04553c91857c9156051d294cf9b`。bootstrap admin、ordinary User、Model API key、DingTalk webhook/signing secret 对全部 57 个 evidence 文件精确扫描零命中；Connector plaintext credential 不可重读，S05 artifact 只保留 Secret 创建结果与安全状态。该 finalized evidence 目录作为 A01 归档，旧失败 run 继续 append-only 保留。

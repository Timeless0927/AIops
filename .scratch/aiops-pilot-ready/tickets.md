...
当前 candidate 首次 clean run（失败证据保留）：`/root/aiops/acceptance/v0.1.0-20260715T051255Z` 绑定 `4cf9a808…8efa`，连续完成 P01-P03/I01-I03/I05/S01-S03；新 Notification Adapter 已令故障 DingTalk Delivery 以三次 durable attempt 达到 `dead_letter`，但 runner 随后在保存真实 config 时同时请求 `enabled: true`，owner 因该新 revision 尚未 test `sent` 返回 `400 notification_configuration_rejected`。修复保持 owner guard 不变，将顺序改为 disabled save→test sent→HITL receipt→enable exact revision→select route；integration fake 现拒绝未验证启用并要求 sent 后 activation，回归先红后绿，A01/Notification 直接消费者 64 passed。该失败 attempt 不覆盖，runner 修复后仍从 clean P01 重来；产品 candidate 不变。

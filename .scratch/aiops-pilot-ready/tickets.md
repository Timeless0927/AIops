...
- [ ] Gateway-owned Change Request/Plan Module 持久化 request、phase state、immutable revisions 和 transition events。
- [ ] User 不提交 YAML、JSON Patch 或 executable proposal；明显 credential 被拒绝并引导 Secure Input。
- [ ] target、desired state、scope 或 post-check 模糊时进入 `needs_input`，新输入 supersede 旧 revision。
- [ ] 模型只收到 sanitized discovery/facts 且无 Kubernetes credential；raw reasoning 不持久化。
- [ ] `/api/v1`、Workbench UI 与 contract tests 覆盖 planning、needs_input、superseded 和 authorization。

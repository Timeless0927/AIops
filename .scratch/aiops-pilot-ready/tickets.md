...
实际发布物为 `dist/aiops-pilot-v0.1.0.tar.gz`（41 KiB）与 `dist/SHA256SUMS`。A01 live acceptance 暴露并修复 Platform Status、Notification、Model readiness 与 remote HTTP client ID 问题后，已用公开可匿名拉取的 Console `9d16af2e…c9102`、Gateway `16c3d1d4…10f7fbd`、Diagnosis `1d3134f0…fe18f` 与 Notification `1a73d5d3…a50444` digest 重建当前 candidate，archive SHA256 为 `4cf9a8080482473ea500aa42db13bc0c89bb6a553ee1b81ea63ae02b93bc8efa`；此前 `59880499…42a`、`fc1b649d…c11a`、`43ca9d4a…13935`、`f0e10b04…0418` 与 `c9c0e106…6600` artifact 均已被 supersede，不得用于 A01。空目录测试验证 safe extraction、checksum、确定性 rebuild、唯一顶层 overlay、本地 render、完整 file/image inventory、zero/tag/Operator CRD 禁止、verification dry-run 与 OpenAPI 全量重新生成逐字节一致；原 package selector 7 passed，Pilot/verification/contract 直接消费者合计 68 passed，本次 candidate 重建后发布/交付 selector 30 passed，TypeScript/Vite production build 通过，仅保留既有 >500 kB warning。归档与 metadata 只声明 clean install/same-version reapply，并明确排除 upgrade、downgrade、backup、rollback 与 data-preserving uninstall；Standards 与 Spec 最终复审均零 finding。
...
当前 live candidate（A01）：`dist/aiops-pilot-v0.1.0.tar.gz` SHA256 `4cf9a8080482473ea500aa42db13bc0c89bb6a553ee1b81ea63ae02b93bc8efa`；Console `sha256:9d16af2e33d63766f57239e64779338f4f8ec0f1c246ae6d355b9ea06c0c9102`、Gateway `sha256:16c3d1d4f112d6fbb7fa6eb89736a9a8dd4b29d2f9c53e6f1b057089f8107fbd`、Diagnosis `sha256:1d3134f08e00bbb29d3366041b56c6b54299b31824c618cdeaf1f7ae7d4fe18f` 与 Notification `sha256:1a73d5d3bf8e2f95b4d2c1f229657765b78628dc2e98c68b6d405af545a50444` 均已通过 split-image import smoke、推送和空 Docker config 匿名 manifest inspect；先前 `f0e10b04…0418` 与 `c9c0e106…6600` candidate 已被 supersede，不得继续用于 A01。直连 `registry.k8s.io` 的 kube-state-metrics manifest 首次 60s 超时后，仅重建进程使用用户授权代理，其余 registry 保持直连；代理未写入项目或 Cluster 配置。

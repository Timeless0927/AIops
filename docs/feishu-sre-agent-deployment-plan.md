...
当前生产入口不直接执行 `hermes gateway`，而是通过父项目 wrapper 启动：

```text
deploy/entrypoint.sh
  |
  |- render ~/.hermes/config.yaml
  |- python3 -m hooks.alert_webhook_server
  `- python3 -m runtime.hermes_gateway
       |
       |- add hermes-agent source path
       |- install runtime.feishu_approval_overlay
       `- call hermes_cli.gateway.run_gateway(replace=True)
```

`AIOPS_WEBHOOK_ONLY=1` 时只启动 Alertmanager webhook server，不启动 Hermes gateway wrapper。这个模式用于排查 Feishu gateway 或 overlay 兼容问题。
...
当前 MVP 只完成 Approval 状态变更，不执行 Remediation：

1. 若分析结果产生建议动作，系统创建 pending approval。
2. 审批状态写入 SQLite，不阻塞其他会话。
3. Thread 摘要给出文本审批指令：`批准 <approval_id>` / `拒绝 <approval_id> <reason>`。
4. 飞书回复由 runtime overlay 拦截并交给 `hooks/approval_reply.py`。
5. 审批结果写回 approval 和 incident timeline。

后续 Remediation 才会处理：

- `k8s_write` / `k8s_exec` 真实执行。
- dry-run。
- operation lock。
- 执行审计。
- 执行后健康检查。
...
每次更新部署镜像或 `hermes-agent` 指针时，必须先验证父项目 wrapper 和 overlay：
...
- `FeishuAdapter._process_inbound_message()` 是否仍存在。
- 方法签名是否仍包含 `data/message/sender_id/chat_type/message_id`。
- overlay 是否仍在 Hermes gateway runner 前安装。
- 普通文本是否仍会进入 Hermes 原始流程。
- 审批文本是否仍会停止在 overlay，不进入 LLM。

回退策略：

- 如果 overlay 安装失败，先回退 Hermes 指针或镜像版本。
- 如果只需要保留 Alertmanager webhook，可临时设置 `AIOPS_WEBHOOK_ONLY=1`。
- 不要为了快速恢复在 `hermes-agent` 子模块内直接写 AIOps 业务补丁。

## 14. 建议的后续实施项

后续开发以 `docs/superpowers/specs/2026-05-07-approval-remediation-execution-complete-design.md` 为完整实现规格。

- 先完成 runtime overlay 生产化收口和文档化。
- 再完成审批人授权。
- 再设计自动执行闭环，包括 dry-run、operation lock、审计、健康检查和 rollback。
- 最后再实现低风险 allowlist 的自动执行、知识沉淀和多实例部署。

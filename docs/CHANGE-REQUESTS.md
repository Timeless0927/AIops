...
本文件保留历史变更请求、产品/架构/测试决策和验证记录。2026-05-20 起，当前任务状态、阻塞项、执行分派和验收结论以 Multica issue 为准；本文件不再作为跨智能体实时协作通信总线。
...
会改变行为的用户反馈仍应在修改代码前形成可追溯记录：优先登记或关联到对应 Multica issue；如该反馈形成长期产品、架构、测试或部署决策，再同步补充到本文件或相应 PDD/BDD/DDD/SDD/TDD 文档。不得依赖本文件替代 issue 状态、阻塞和验收回写。
...
- TDD：必须补回归测试。
- TODD：历史要求为同步当前任务、进展和验证记录；2026-05-20 事实源迁移后，后续同类状态回写 Multica issue，仅长期知识更新文档。

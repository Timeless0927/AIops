...
| Hermes -> Gateway: diagnosis artifact writeback | Hermes 与 Gateway 配置相同 `AIOPS_GATEWAY_WRITEBACK_SECRET`，Hermes 完成 `diagnosed/partial` session | `POST /diagnosis/writeback` 与 `GET /incidents/{incident_id}` 均要求 `X-AIOPS-Writeback-Signature` HMAC，未配置/缺失/错误签名 fail-closed 返回 401；认证成功后 Gateway durable incident row 可取回 diagnosis JSON/Markdown/summary/confidence/diagnosed_at 与 timeline refs | `tests/test_hermes_diagnosis_service.py::test_split_store_diagnosis_writeback_persists_gateway_incident_artifacts`, `tests/test_gateway_alertmanager_webhook.py::test_gateway_writeback_http_requires_signature`, `tests/test_gateway_alertmanager_webhook.py::test_gateway_writeback_http_accepts_valid_signature_and_protects_incident_view` |
| Hermes writeback 失败记录语义 | Gateway 不可用、未配置 writeback secret、或 split-store 下 Hermes 本地没有 incident row | 诊断 HTTP export 继续可用；失败信息保留在 Hermes session export 与 `/diagnosis/sessions/{id}/timeline` export 的 `writeback` 字段。该场景不保证 durable Gateway audit，因为 Gateway 不可用或未接受请求时无法写入 durable store | `tests/test_hermes_diagnosis_service.py::test_gateway_writeback_failure_keeps_session_export_available`, `tests/test_hermes_diagnosis_service.py::test_gateway_writeback_without_secret_fails_closed_without_http_request` |

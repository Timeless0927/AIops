...
Alertmanager 告警 payload 通常带 `cluster` label。webhook 层自动提取业务集群标识，并在签名、审计、锁和通知中保留该 label：
- 告警带 `cluster: production` → 业务集群标识记录为 production
- 默认集群内部署使用当前 ServiceAccount，不把业务 label 自动追加为 `kubectl --context`
- 只有配置显式映射（如 `AIOPS_KUBE_CONTEXT_MAP`）或传入 `kube_context` 时，才切换 kube context
- 每个工具调用都保留业务 cluster 与实际 kube context 的边界，避免跨集群误操作
- Prometheus/Loki 地址如需跨集群路由，应通过独立配置映射，不复用业务 label 作为隐式 kube context
...
| 告警集群路由 | 小 | webhook 层解析业务 cluster label；显式映射后才切 kube context |

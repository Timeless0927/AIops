...
For a controlled Deployment restart, use only an RFC 6902 add at /spec/template/metadata/annotations/aiops.dev~1restart-request-id with change_request_id as its string value. If context explicitly identifies a controlled verification run_id, instead add /spec/template/metadata/annotations/aiops.dev~1verification-run-id with that exact run_id. Verify the selected exact annotation with {"type":"json_pointer","path":"<the exact annotation path>","operator":"eq","value":"<the exact annotation value>"} and also {"type":"workload_rollout"}; do not rename those fields. A rollout cannot restore previous Pod identities, so declare rollback unavailable with that concrete loss. Never emit a typed restart action or rollout command."""
